...
namespace USherbrooke.ServiceModel.Sondage
{
    ...
    public class SimpleSondageDAO : ISondageDAO
    {
        ...
        private readonly IDictionary<int, int> lastAnsweredQuestion = new Dictionary<int, int>();
        ...
        public int GetLastQuestionAnswered(int pollId)
        {
            int result = -1;
            lastAnsweredQuestion.TryGetValue(pollId, out result);
        ...
        public PollQuestion GetNextQuestion(int pollId, int currentQuestionId)
        {
            ...
            if (availablePolls.TryGetValue(pollId, out questions) && questions.Count > 0)
            {
                ...
                {
                    lastAnsweredQuestion[pollId] = questions[0].QuestionId;
                    return questions[0];
                }
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    {
                        lastAnsweredQuestion[pollId] = questions[i].QuestionId;
